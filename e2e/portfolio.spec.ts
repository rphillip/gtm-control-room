...
test('Hospital TAM deep link reloads with honest synthetic and source framing', async ({ page }) => {
  ...
  await expect(machine.locator('[data-artifact-data-ball]')).toHaveCSS('animation-duration', '4.8s')
  await expect(machine.locator('.tam-collapse-gauge__hopper')).toHaveCSS('animation-name', 'tam-hopper-cycle')
  await expect(machine.locator('.tam-collapse-gauge__lifter')).toHaveCSS('animation-duration', '4.8s')
  await page.locator('#walkthrough').getByRole('button', { name: /Join to health systems/i }).click()
  await expect(machine.locator('.tam-collapse-gauge__join span').first()).toHaveCSS('animation-name', 'tam-jaw-left-cycle')
  await expect(machine.locator('.tam-collapse-gauge__join-key')).toHaveCSS('animation-duration', '4.8s')
  await page.locator('#walkthrough').getByRole('button', { name: /Resolve to GTM companies/i }).click()
  await expect(machine.locator('.tam-collapse-gauge__rollers')).toHaveCSS('animation-name', 'tam-rollers-cycle')
  ...
  expect(await machine.locator('.tam-collapse-gauge__spring').evaluate((element) => getComputedStyle(element, '::after').animationName)).toBe('tam-spring-cycle')
  await expect(machine.locator('.tam-collapse-gauge__spring span').first()).toHaveCSS('animation-name', 'tam-counter-one')
