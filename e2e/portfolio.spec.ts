...
test('Hospital TAM deep link reloads with honest synthetic and source framing', async ({ page }) => {
  ...
  await expect(page.getByText('No data background needed.')).toHaveCount(0)
  await expect(page.getByRole('heading', { name: /She needs a list of organizations that could buy her product/i })).toBeVisible()
  await expect(page.locator('.tam-identity-legend__machine')).toContainText('Hospital')
  await expect(page.locator('.tam-identity-legend__machine')).toContainText('Health system')
  await expect(page.locator('.tam-identity-legend__machine')).toContainText('Sales account')
  await expect(page.locator('.tam-identity-legend__ball')).toHaveCSS('animation-name', 'tam-legend-ball')
  await expect(page.getByText(/Every name, identifier, relationship, and domain.*invented/i)).toBeVisible()
  await expect(page.getByText(/not a universal score or a prediction/i)).toBeVisible()
  ...
  await expect(machine.locator('.tam-account-counter__balls i')).toHaveCount(3)
  await expect(machine.locator('.tam-account-counter__counted b')).toHaveCount(3)
  await expect(machine.locator('[data-artifact-data-ball]')).toHaveCSS('animation-duration', '9.6s')
  await expect(machine.locator('.tam-collapse-feeder__drum')).toHaveCSS('animation-name', 'tam-feed-drum-cycle')
  await page.locator('#walkthrough').getByRole('button', { name: /Join to health systems/i }).click()
  await expect(machine.locator('.tam-system-resolver__press')).toHaveCSS('animation-name', 'tam-system-press-cycle')
  await expect(machine.locator('.tam-system-resolver__balls b').first()).toHaveCSS('animation-name', 'tam-system-ball-cycle')
  await page.locator('#walkthrough').getByRole('button', { name: /Resolve to GTM companies/i }).click()
  await expect(machine.locator('.tam-company-resolver__gear--a')).toHaveCSS('animation-name', 'tam-company-gear-a')
  await expect(machine.locator('.tam-company-resolver__outputs b').first()).toHaveCSS('animation-name', 'tam-company-output-cycle')
  await page.locator('#walkthrough').getByRole('button', { name: /Now count prospects/i }).click()
  await expect(machine.locator('.tam-account-counter__balls i').first()).toHaveCSS('animation-name', 'tam-counter-ball-one')
  await expect(machine.locator('.tam-account-counter__paddles b').nth(2)).toHaveCSS('animation-name', 'tam-counter-paddle-three')
  await expect(machine.locator('.tam-account-counter__display i').first()).toHaveCSS('animation-name', 'tam-counter-display-cycle')
  await expect(page.getByText(/review queue—not force them/i)).toBeVisible()
  const passport = page.locator('.tam-passport-machine')
  await passport.getByRole('button', { name: /Advance record/i }).click()
  await passport.getByRole('button', { name: /Advance record/i }).click()
  await expect(passport.getByRole('status')).toContainText('Identity station 3 of 7')
  await passport.getByText(/View the technical data receipt/i).click()
  await passport.getByRole('switch', { name: /Simulate a disputed match/i }).check()
  await expect(passport).toContainText('CONFLICT')
  await expect(passport).toContainText('HUMAN REVIEW')
  await passport.getByRole('switch', { name: /Simulate a disputed match/i }).uncheck()
  await passport.getByRole('button', { name: /Advance record/i }).click()
  await expect(passport).toContainText('CO001')
  const thesisMachine = page.locator('.tam-thesis-machine')
  await expect(thesisMachine.getByRole('status')).toContainText('include for investigation')
  await thesisMachine.getByRole('radio', { name: /Rural-care operations/i }).check()
  await expect(thesisMachine.getByRole('status')).toContainText('not a fit for this product strategy')
  await thesisMachine.getByRole('combobox', { name: /Rural-care operating evidence/i }).selectOption('missing')
  await expect(thesisMachine.getByRole('status')).toContainText('needs human review')
  const scoreBuilder = page.locator('.tam-score-builder')
  await expect(scoreBuilder).toHaveCount(1)
  await expect(scoreBuilder.locator('.tam-score-builder__data-ball')).toHaveCount(1)
  await page.getByRole('radio', { name: '10+ hospitals' }).check()
  await expect(scoreBuilder.locator('.tam-builder-machine__ball')).toHaveCSS('animation-name', 'builder-ball-size-three')
  await page.getByRole('button', { name: /Set organization size and roll onward/i }).click()
  await expect(scoreBuilder.getByRole('status')).toContainText('Account brief station 2 of 4')
  await page.getByRole('button', { name: /Set operating footprint and roll onward/i }).click()
  await page.getByRole('button', { name: /Set hospital mix and roll onward/i }).click()
...
test('Hospital TAM supports keyboard controls, reduced motion, forced colors, and 200% zoom', async ({ page }) => {
  ...
  expect(await page.locator('[data-artifact-data-ball]').evaluateAll((elements) => elements.every((element) => (Number.parseFloat(getComputedStyle(element).animationDuration) || 0) < 0.01))).toBe(true)
  expect(await page.locator('.tam-passport-track__ball').evaluate((element) => Number.parseFloat(getComputedStyle(element).transitionDuration) || 0)).toBeLessThan(0.01)
  expect(await page.locator('.tam-thesis-machine__ball').evaluate((element) => Number.parseFloat(getComputedStyle(element).animationDuration) || 0)).toBeLessThan(0.01)
  expect(await page.locator('.tam-builder-machine__ball').evaluate((element) => Number.parseFloat(getComputedStyle(element).animationDuration) || 0)).toBeLessThan(0.01)
  expect(await page.locator('.tam-score-builder__data-ball').evaluate((element) => Number.parseFloat(getComputedStyle(element).transitionDuration) || 0)).toBeLessThan(0.01)
  ...
  await expect(page.locator('#checklist').getByRole('status')).toContainText('1 of 5 market checks complete')
