...
test('Hospital TAM deep link reloads with honest synthetic and source framing', async ({ page }) => {
  ...
  await passport.getByRole('switch', { name: /Introduce ambiguity/i }).check()
  await expect(passport).toContainText('CONFLICT')
  await expect(passport).toContainText('HUMAN REVIEW')
  await passport.getByRole('switch', { name: /Introduce ambiguity/i }).uncheck()
  await passport.getByRole('button', { name: /Advance record/i }).click()
  ...
  await page.getByRole('button', { name: /Lock size and roll onward/i }).click()
  await expect(scoreBuilder.getByRole('status')).toContainText('Scoring station 2 of 4')
  await page.getByRole('button', { name: /Lock geography and roll onward/i }).click()
  await page.getByRole('button', { name: /Lock facility mix and roll onward/i }).click()
  await page.getByRole('checkbox', { name: 'Payer Contracting' }).check()
  await page.getByRole('button', { name: /Print the account profile/i }).click()
