...
describe('HospitalTamPage', () => {
  it('frames the project, audience, sources, limitations, and non-endorsement honestly', () => {
    ...
    expect(screen.queryByText('No data background needed.')).not.toBeInTheDocument()
    expect(screen.getByRole('heading', { name: /She needs a list of organizations that could buy her product/i })).toBeVisible()
    expect(screen.getByText(/The red ball is one piece of data/i)).toBeVisible()
    expect(screen.getAllByText(/Maya’s market puzzle/i)).toHaveLength(5)
    expect(screen.getByText(/total revenue opportunity if every suitable organization/i)).toBeVisible()
