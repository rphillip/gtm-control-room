...
export function MetricStrip({ metrics }: { metrics: Metric[] }) {
  ...
  return (
    <ul className="metric-strip" aria-label="Selected system evidence">
      <li className="metric-strip__provenance-note">{hasUnavailable ? 'Unavailable means the sanitized snapshot did not provide this dynamic field.' : 'Observed aggregate evidence; samples are labeled.'}</li>
      {metrics.map((metric) => (
