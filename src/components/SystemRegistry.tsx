...
function topologyText(workflow: ClaySnapshot['workflows'][number]) {
  ...
  return ordered.length === workflow.nodes.length
    ? `Linear sequence: ${ordered.join(' → ')}`
    : `Connected topology: ${workflow.edges.map(([from, to]) => `${workflow.nodes[from].name} → ${workflow.nodes[to].name}`).join('; ')}`
}
